...
void print_usage() {
    std::cout << "Usage: cpu-sim [options] filename\n"
                 "\n"
                 "Options\n"
                 "   -h, --help:\n"
                 ...
                 "\n"
                 "   -s, --time_slice <value>:\n"
                 "       Set the default time slice for a pre-emptive "
                 ...
                 "\n"
                 "   -v, --verbose:\n"
                 "       If set, outputs all state transitions and scheduling "
                 "choices.\n"
                 "\n"
...
int parse_flags(int argc, char *const argv[], FlagOptions &flags) {
    ...
    while (true) {
        flag_char =
                getopt_long(argc, argv, "-s:tvhma:", flag_options, &option_index);
    ...
    if ((flags.scheduler == "FCFS" || flags.scheduler == "PRIORITY") &&
        (flags.time_slice != -1)) {
        return 1;
    }
...
std::string get_scheduler() {
    ...
    std::transform(input_algorithm.begin(), input_algorithm.end(),
                   input_algorithm.begin(), ::toupper);
    std::string valid_algorithms[] = {"FCFS", "RR", "PRIORITY", "MLFQ", "CUSTOM"};

    bool valid =
            std::find(std::begin(valid_algorithms), std::end(valid_algorithms),
                      input_algorithm) != std::end(valid_algorithms);
